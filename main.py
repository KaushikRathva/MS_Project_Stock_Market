...
class HMM:
    ...
    # def __init__(self, n_states, n_ob_symbols):
        ...
        # self.n_ob_symbols = n_ob_symbols

        # self.transition_prob_mat = np.eye(self.n_states)
        # self.emission_prob_mat = np.full((self.n_states, self.n_ob_symbols), 1.0 / self.n_ob_symbols)
        # self.stationary_dist = np.full(self.n_states, 1.0 / self.n_states)
...
def ready_data(data_csv):
    ...
    start_index = np.random.randint(0, len(data_dict['Open']) - 350)

    train_data = {key: values[start_index:start_index + 300] for key, values in data_dict.items()}
    test_data = {key: values[start_index + 300:start_index + 310] for key, values in data_dict.items()}
...
def main():
    ...
    open_price_train_trend,possible_observation = find_trands(train_data, 25)
    ...
    open_price_test_trend,possible_observation = find_trands(test_data, 25)
